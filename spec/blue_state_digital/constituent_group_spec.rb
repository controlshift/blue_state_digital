...
describe BlueStateDigital::ConstituentGroup do
  it "should create constituent group" do
    ...
    output_empty = %q{<?xml version="1.0" encoding="utf-8"?>}
    output_empty << "<api>"
    output_empty << "</api>"
    ...
    BlueStateDigital::Connection.should_receive(:perform_request).with('/cons_group/get_constituent_group_by_name', {name:attrs[:name]}, "GET") { output_empty }
  ...
  it "should not create constituent group if group already exist" do
    timestamp = Time.now.to_i
    attrs = { name: "Environment", slug: "environment", description: "Environment Group", group_type: "manual", create_dt: timestamp }
    input = %q{<?xml version="1.0" encoding="utf-8"?>}
    input << "<api>"
    ...
    BlueStateDigital::Connection.should_receive(:perform_request).with('/cons_group/get_constituent_group_by_name', {name:attrs[:name]}, "GET") { output }
    BlueStateDigital::Connection.should_not_receive(:perform_request).with('/cons_group/add_constituent_groups', {}, "POST", input)
